...
struct Running3DView: UIViewRepresentable {
    
    var speed: Double // Running Speed in MPH
    ...
    class Coordinator {
        ...
        func setupAnimationTimer(speed: Double, modelEntity: ModelEntity) {
            let interval = (speed / 10)
    ...
    func makeUIView(context: Context) -> ARView {
        ...
        let filename = "RunningAnimation.usdz"
        
        if let modelEntity = try? ModelEntity.loadModel(named: filename) {
            modelEntity.scale = [0.9, 0.9, 0.9]
            modelEntity.position = [0, -0.65, 0]
            
            let anchorEntity = AnchorEntity()
            addAllChildren(to: anchorEntity, from: modelEntity)
            arView.scene.addAnchor(anchorEntity)
            
            context.coordinator.setupAnimationTimer(speed: speed, modelEntity: modelEntity)
